...
- **Organigrama Vivo (`/os#/org`) — eliminar y desvincular**: la implementación 2.0 había perdido las acciones de borrado de la 1.x, así que un elemento agregado no se podía quitar. Se reponen en el inspector (botón **Eliminar**), en el menú de acciones contextuales (**Desvincular del padre**, **Eliminar elemento**) y en el inspector de relación (**Desvincular**). El borrado es ordenado: elimina relaciones, Role Cards y limpia los vínculos que apuntaban al nodo (KPIs, procesos, SOPs); si el nodo tiene elementos debajo, se elige explícitamente entre quitarlo solo o con todo lo que cuelga. Nunca borra documentos ERPNext y, si el rol del usuario no permite eliminar (se requiere `OS Admin`/`System Manager`), lo explica en lugar de fallar en silencio.
