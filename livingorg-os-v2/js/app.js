...
(function () {
  "use strict";
  const LO = window.LO;
  /* Tipos de relación entre nodos del organigrama (Modo Unir) */
  LO.REL_TYPES = {
    REPORTS_TO: { label:"Reporta a",        color:"#0FA3A3", icon:"org",   desc:"Depende jerárquicamente del nodo destino" },
    OWNS:       { label:"Posee / gestiona", color:"#C8862F", icon:"layers", desc:"Es dueño o responsable del nodo destino" },
    EXECUTES:   { label:"Ejecuta",          color:"#6D5AE6", icon:"zap",   desc:"Ejecuta procesos o tareas del nodo destino" },
  };
  ...
  /* Repara/normaliza datos guardados por versiones viejas de la app:
     nodos sin coordenadas (se ven apilados y sus líneas son invisibles),
     conexiones inválidas y entidades faltantes. */
  function repair(db){
    db=db||{};
    const s=seed();
    Object.keys(s).forEach(k=>{ if(!Array.isArray(db[k])) db[k]=s[k]; });
    const ons=db.orgnode||[];
    ons.forEach((n,i)=>{ if(typeof n.x!=="number"||!isFinite(n.x)||typeof n.y!=="number"||!isFinite(n.y)){
      n.x=80+(i%4)*260; n.y=80+Math.floor(i/4)*200; } });
    db.relation=(db.relation||[]).filter(r=>r&&r.from&&r.to);
    db.flujo=(db.flujo||[]).filter(f=>f&&f.proceso);
    (db.flujo||[]).forEach(f=>{
      f.nodes=(f.nodes||[]).filter(n=>n&&n.id);
      f.nodes.forEach(n=>{ if(typeof n.x!=="number"||!isFinite(n.x)) n.x=60+Math.random()*320;
                           if(typeof n.y!=="number"||!isFinite(n.y)) n.y=60+Math.random()*260; });
      f.edges=(f.edges||[]).filter(e=>e&&e.from&&e.to);
    });
  }
  function load(){
    if (mem) return mem;
    if (canLS){ try{ const r=localStorage.getItem(KEY); if(r){ mem=JSON.parse(r); repair(mem); persist(); return mem; } }catch(e){} }
  ...
  function openDetail(entityKey, id){
    ...
    if (entityKey==="orgnode"){
      body.appendChild(el("div",{class:"section-title"},"Conexiones"));
      const rels=Store.all("relation").filter(r=>r.from===id||r.to===id);
      if(!rels.length){ body.appendChild(el("div",{class:"fv empty"},"Sin conexiones. Activa “Unir nodos” en el canvas para conectar.")); }
      else {
        const list=el("div",{class:"conn-list"});
        rels.forEach(r=>{
          const outbound = r.from===id;
          const otherId = outbound? r.to : r.from;
          const other = Store.get("orgnode", otherId);
          const t = LO.REL_TYPES[r.type]||{label:r.type||"RELACIÓN", color:"#8593A2"};
          const row=el("div",{class:"conn-row"},[
            el("span",{class:"badge", style:{color:t.color, background:t.color+"14", borderColor:t.color+"33"}}, t.label),
            el("span",{class:"conn-arrow", title: outbound? "Sale de este nodo":"Entra a este nodo"}, outbound? "→":"←"),
            el("span",{class:"conn-name", title:"Ver nodo", onclick:()=>{ closeDrawer(); openDetail("orgnode", otherId); }}, (other&&other.name)||otherId),
            el("button",{type:"button", class:"conn-del", title:"Eliminar conexión", onclick:()=>{
              Store.remove("relation", r.id); openDetail("orgnode", id); LO.Router.refresh(); toast("Conexión eliminada"); }}, icon("trash")),
          ]);
  ...
  function orgView(){
    ...
    /* ---- MODO UNIR (conectar nodos) ---- */
    const REL_TYPES = LO.REL_TYPES || {};
    let connectMode=false, connectFrom=null, previewPath=null, freshEdge=null, hoverNode=null;
    const hintEl = el("div",{class:"org-hint"});
    function updateHint(){
      hintEl.innerHTML="";
      hintEl.appendChild(icon(connectMode?"link":"move"));
      hintEl.appendChild(document.createTextNode(
        connectMode
          ? (connectFrom? ("Origen: “"+connectFrom.name+"” — clic en el nodo destino para conectar · Esc para salir")
                       : "Modo unir: clic en el nodo origen y luego en el destino · Esc para salir")
          : "Arrastra los nodos · clic para ver el contrato · arrastra el fondo para desplazar"));
    }
    function setSource(rec){
      connectFrom=rec;
      canvas.querySelectorAll(".node").forEach(n=>n.classList.toggle("sel", !!rec && n.dataset.id===rec.id));
      if(!rec) hidePreview();
      updateHint();
    }
    ...
    function exitConnect(){ if(connectMode) toggleConnect(); }
    function hidePreview(){ if(previewPath){ previewPath.setAttribute("d",""); previewPath.style.display="none"; } }

    const connectBtn = el("button",{class:"btn sm", title:"Unir nodos (conectar dos nodos con una relación)", onclick:toggleConnect},[icon("link"),"Unir nodos"]);
    ...
    function drawEdges(){
      ...
      defs.innerHTML='<marker id="arrO" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0 0L10 5L0 10z" fill="var(--edge)"/></marker>'
        +'<marker id="arrOb" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0 0L10 5L0 10z" fill="var(--brand)"/></marker>';
      ...
      rels.forEach(r=>{
        ...
        path.setAttribute("class","edge"+(exec?" exec":"")+(freshEdge===r.id?" fresh":""));
        path.setAttribute("d",`M${x1} ${y1} C ${mx} ${y1}, ${mx} ${y2}, ${x2} ${y2}`);
        path.setAttribute("marker-end", exec? "url(#arrOb)":"url(#arrO)");
        svgEl.appendChild(path);
        const t=document.createElementNS("http://www.w3.org/2000/svg","text");
        t.setAttribute("class","edge-label"); t.setAttribute("x",mx); t.setAttribute("y",my-5);
        t.setAttribute("text-anchor","middle"); t.textContent=r.type||"";
        svgEl.appendChild(t);
        // zona invisible amplia para hover/editar/eliminar la conexión
        const hit=document.createElementNS("http://www.w3.org/2000/svg","path");
        hit.setAttribute("class","edge-hit");
        hit.setAttribute("d",`M${x1} ${y1} C ${mx} ${y1}, ${mx} ${y2}, ${x2} ${y2}`);
        hit.addEventListener("pointerenter",()=>path.classList.add("hover"));
        hit.addEventListener("pointerleave",()=>path.classList.remove("hover"));
        hit.addEventListener("click",e=>{ e.stopPropagation(); edgeMenu(r, mx, my); });
        svgEl.appendChild(hit);
      });
      previewPath=document.createElementNS("http://www.w3.org/2000/svg","path");
      previewPath.setAttribute("class","edge preview"); previewPath.style.display="none";
      svgEl.appendChild(previewPath);
    }
    function edgeMenu(r, wx, wy){
      const rect=canvas.getBoundingClientRect();
      const sx=rect.left+wx*scale, sy=rect.top+wy*scale;
      const anchor={ getBoundingClientRect:()=>({left:sx,right:sx,top:sy,bottom:sy}), contains:()=>false };
      menu(anchor,{ items:[
        {header:"Conexión · "+r.type},
        ...Object.keys(REL_TYPES).map(k=>({
          label:"Cambiar a "+REL_TYPES[k].label, icon:"link", color:REL_TYPES[k].color, on:r.type===k,
          onClick:()=>{ Store.upsert("relation",{id:r.id, type:k}); drawEdges(); toast("Conexión actualizada ✓"); } })),
        {sep:true},
        {label:"Eliminar conexión", icon:"trash", color:"var(--err)",
          onClick:()=>{ Store.remove("relation", r.id); drawEdges(); toast("Conexión eliminada"); }},
      ]});
    }
    function paintNodes(){
      ...
      nodes.forEach(n=>{
        ...
        const node=el("div",{class:"node","data-id":n.id, style:{left:(n.x||40)+"px", top:(n.y||40)+"px"}});
    ...
    function enableDrag(node, rec){
      ...
      node.addEventListener("pointerdown",e=>{
        ...
        if(connectMode){
          // modo unir: primer toque selecciona origen; arrastre hacia el destino
          try{ node.setPointerCapture(e.pointerId); }catch(err){}
          if(!connectFrom) setSource(rec);
          return;
        }
      ...
      node.addEventListener("pointerup",e=>{
        if(connectMode){ handleConnectClick(rec); return; }
    ...
    function handleConnectClick(rec){
      const to = hoverNode && hoverNode.id!==rec.id ? hoverNode : null;
      if(!connectFrom){ if(!to) setSource(rec); return; }
      if(connectFrom.id===rec.id){ if(!to) return; }
      const from=connectFrom;
      setSource(null);
      askRelType(from, to || rec);
    }
    function askRelType(fromRec, toRec){
      const opts=el("div",{class:"rel-options"}, Object.keys(REL_TYPES).map(k=>{
        const t=REL_TYPES[k];
        const exists=Store.all("relation").some(r=>r.from===fromRec.id&&r.to===toRec.id&&r.type===k);
        return el("button",{type:"button", class:"rel-opt"+(exists?" has":""), onclick:()=>{
          if(exists){ toast("Esa conexión ya existe", true); }
          else { const saved=Store.upsert("relation",{from:fromRec.id, to:toRec.id, type:k}); freshEdge=saved.id; toast("Conexión creada ✓"); }
          closeModal(); drawEdges();
          setTimeout(()=>{ freshEdge=null; if(document.contains(wrap)) drawEdges(); }, 1600);
        }},[
          el("span",{class:"ro-ico", style:{background:t.color+"1f", color:t.color}}, icon(t.icon||"link")),
          el("span",{class:"ro-t"}, t.label),
          el("span",{class:"ro-d"}, "“"+fromRec.name+"” → “"+toRec.name+"” · "+t.desc),
          exists? el("span",{class:"ro-ex"},[icon("check"),"Ya existe"]):null,
        ]);
      }));
      const sheet=el("div",{class:"sheet rel-sheet"},[
        el("div",{class:"sheet-head"},[
          el("div",{class:"eyebrow"},[ el("span",{class:"di", style:{background:"#6D5AE6"}}, icon("link")), "Unir nodos" ]),
          el("h2",{}, "Conectar “"+fromRec.name+"” con “"+toRec.name+"”"),
          el("p",{}, "Elige el tipo de relación. El sentido va de “"+fromRec.name+"” hacia “"+toRec.name+"”."),
        ]),
        el("div",{class:"sheet-body"}, opts),
        el("div",{class:"sheet-foot"},[
          el("button",{class:"btn ghost", onclick:closeModal},"Cancelar"),
          el("div",{class:"spacer"}),
          el("button",{class:"btn", onclick:()=>{ closeModal(); askRelType(toRec, fromRec); }},[icon("reset"),"Invertir dirección"]),
        ]),
      ]);
      modal.innerHTML=""; modal.appendChild(sheet);
      requestAnimationFrame(()=>{ modal.classList.add("show"); scrim.classList.add("show"); });
    ...
    // vista previa de la conexión siguiendo el cursor / nodo bajo el puntero
    canvas.addEventListener("pointermove",e=>{
      if(!connectMode) return;
      const elAt=document.elementFromPoint(e.clientX, e.clientY);
      const nEl=elAt? elAt.closest(".node") : null;
      const hover=nEl? Store.get("orgnode", nEl.dataset.id) : null;
      hoverNode=hover||null;
      canvas.querySelectorAll(".node").forEach(n=>
        n.classList.toggle("hover", n===nEl && !!hover && (!connectFrom || hover.id!==connectFrom.id)));
      if(!connectFrom || !previewPath){ hidePreview(); return; }
      const rect=canvas.getBoundingClientRect();
      const wx=(e.clientX-rect.left)/scale, wy=(e.clientY-rect.top)/scale;
      let x2=wx, y2=wy;
      if(hover && hover.id!==connectFrom.id){ x2=(hover.x||40)+98; y2=(hover.y||40)+46; }
      const x1=(connectFrom.x||40)+98, y1=(connectFrom.y||40)+46, mx=(x1+x2)/2;
      previewPath.setAttribute("d",`M${x1} ${y1} C ${mx} ${y1}, ${mx} ${y2}, ${x2} ${y2}`);
      previewPath.style.display="";
    });
    // desplazar el fondo (pan) — y en modo unir, clic al fondo quita la selección
    let panning=false,panMoved=false,psx,psy,ppx,ppy;
    canvas.parentElement.addEventListener("pointerdown",e=>{
      if(e.target.closest(".node")||e.target.closest(".edge-hit")||e.target.closest(".org-toolbar")||e.target.closest(".org-legend")) return;
      panning=true; panMoved=false; psx=e.clientX; psy=e.clientY; ppx=panX; ppy=panY;
    });
    window.addEventListener("pointermove",e=>{ if(!panning)return;
      ...
      panX=ppx+(e.clientX-psx); panY=ppy+(e.clientY-psy); apply(); });
    window.addEventListener("pointerup",()=>{
      if(panning && !panMoved && connectMode && connectFrom) setSource(null);
      panning=false;
    });
    window.addEventListener("keydown",e=>{
      if(e.key!=="Escape" || !connectMode || !document.contains(wrap)) return;
      if(!modal.classList.contains("show")) exitConnect();
    });
